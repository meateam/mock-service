...
import (
	...
	"github.com/meateam/mock-service/service"
...
// MockServer is a structure that holds the mock grpc server
...
func (s MockServer) Serve(lis net.Listener) {
	listener := lis
	if lis == nil {
...
// NewServer configures and creates a grpc.Server instance with the mock service
...
func NewServer(logger *logrus.Logger) *MockServer {
	...
	// Create a mock service and register it on the grpc server.
	mockService := service.NewService(logger)
	pb.RegisterMockServer(grpcServer, mockService)
	...
	go mockServer.healthCheckWorker(healthServer)

	return mockServer
}

// serverLoggerInterceptor configures the logger interceptor for the mock server.
...
func (s MockServer) healthCheckWorker(healthServer *health.Server) {
	for {
